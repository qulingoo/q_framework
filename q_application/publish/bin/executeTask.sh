...
JAVA_OPTS="-Duser.timezone=GMT+8 -server -Xms128m -Xmx512m"

#Java程序日志所在的目录
APP_LOG=../log

[[ -d $APP_LOG ]] || mkdir $APP_LOG

#Java程序主体所在的目录,即classes的上一级目录
APP_HOME=../

#Java主程序,即main(String[] args)方法类
APP_MAIN=com.shuyun.Application

#classpath参数,包括指定lib目录下的所有jar
CLASSPATH=$APP_HOME/classes:$APP_HOME/config
...
start(){
    ...
    if [ $tradePortalPID -ne 0 ]; then
        ...
        nohup $JAVA_HOME/bin/java $JAVA_OPTS -classpath $CLASSPATH $APP_MAIN > $APP_LOG/main.log &
        getTradeProtalPID
        if [ $tradePortalPID -ne 0 ]; then
